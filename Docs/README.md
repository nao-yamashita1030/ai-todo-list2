...
このリポジトリには3つのモードがあります：

#### 開発モード
- **トリガー**: 「開発モードスタート」というプロンプト
- **用途**: 実際のシステム開発を進める
- **制約**: `.cursor/rules/` ディレクトリ内のルールファイルやテンプレートファイルは読み取り専用
...
#### フィードバックモード
- **トリガー**: 「フィードバックモードスタート」というプロンプト
- **用途**: 基盤リポジトリに対する改善点・課題点を記録する
- **動作**: ユーザーが入力した内容を `Docs/feedback/` ディレクトリに記録
...
**重要**: `status.md` の「最終更新日時」を更新する際は、必ず本日の日付を取得して設定してください。

- **日付の取得方法**: Windows環境では、PowerShellコマンド `Get-Date -Format 'yyyy年MM月dd日'` を使用して本日の日付を取得できます
- **日付の形式**: `yyyy年MM月dd日` の形式で記載してください（例: `2025年12月4日`）
- **更新タイミング**: `status.md` を更新する際は、必ず最終更新日時も更新してください
- **日付の確認**: 日付を設定する前に、必ず本日の日付を確認してください（手動で日付を入力するのではなく、システムから取得した日付を使用してください）
...
### フィードバックモードでのドキュメント管理

フィードバックモードでは、`Docs/feedback/` ディレクトリにフィードバックを記録します：
- ユーザーが入力した内容を構造化して記録
- テンプレートを使用してフィードバックファイルを作成
- 記録したフィードバックは、計画モードで参照して基盤リポジトリの改善に活用

## 関連ファイル

- `.cursor/rules/`: AIコンサルタントの動作設定（プロジェクトルール、モード判定ロジックを含む）
